...
import { setCouponStatus } from "@/app/admin/cupons/actions";
import { Button } from "@/components/ui/button";
import { getPendingCoupons } from "@/lib/coupons";

export default async function ModerarCuponsPage() {
  const pending = await getPendingCoupons();
  return (
    <div>
      <h2 className="mb-3 text-lg font-semibold">Cupons pendentes</h2>
      {pending.length === 0 ? (
