import base64, pathlib, html as _h

ROOT = pathlib.Path("/Users/rodrigokrug/melhorfilamento")
OUT = pathlib.Path("/tmp/pecas"); OUT.mkdir(exist_ok=True)
b64 = lambda p: base64.b64encode((ROOT / p).read_bytes()).decode()
LOGO_DARK = b64("public/img/logo-dark.png")
LOGO_COLOR = b64("public/img/logo-color.png")

# tema: (bg, text, sub, eyebrow, accent_g, hl, tagline_hl, logo)
THEMES = {
 "navy": dict(bg="background:radial-gradient(900px 600px at 85% -10%,rgba(84,182,46,.18),transparent 60%),radial-gradient(800px 700px at -10% 25%,rgba(14,126,123,.4),transparent 55%),#0C1F23;",
 ...
 "grad": dict(bg="background:linear-gradient(135deg,#0E7E7B 0%,#54B62E 100%);",
   text="#fff", sub="rgba(255,255,255,.92)", eye="rgba(255,255,255,.92)", g="#0C1F23", taghl="#0C1F23", logo="white"),
 "teal": dict(bg="background:linear-gradient(150deg,#0A6967 0%,#063432 100%);",
...
def mark_svg(hole, lupa, topc):
    return (
      ...
      '<stop offset="0%" stop-color="#16ABA6"/><stop offset="100%" stop-color="#54B62E"/>'
      '</linearGradient></defs>'
      ...
      f'<circle cx="21.4" cy="21" r="6.2" fill="{hole}"/>'
...
def logo_html(t):
    if t["logo"] == "mark":
        ...
        return (
          '<div style="display:flex;align-items:center;gap:14px">' + svg +
          f'<div style="font-family:Sora;font-weight:700;font-size:34px;letter-spacing:-.02em;color:{t["text"]}">'
          f'Melhor<span style="color:{t["wm2"]}">Filamento</span></div></div>')
    # white wordmark (texto) — usado em fundos grad/teal
    return ('<div style="font-family:Sora;font-weight:700;font-size:38px;color:#fff;letter-spacing:-.02em">'
...
.card{{width:1080px;height:1080px;{bg}padding:92px;display:flex;flex-direction:column;justify-content:space-between;font-family:Manrope,sans-serif}}
.eye{{font-family:'Space Mono',monospace;font-weight:700;font-size:24px;letter-spacing:.16em;text-transform:uppercase;color:{eye}}}
h1{{font-family:Sora,sans-serif;font-weight:800;font-size:84px;line-height:1.04;letter-spacing:-.03em;color:{text};margin:26px 0 0}}
h1 .g{{color:{g}}}
h1 u{{text-decoration-color:{g};text-underline-offset:8px;text-decoration-thickness:7px}}
.sub{{font-size:36px;line-height:1.4;color:{subcol};margin-top:30px;max-width:840px}}
.bar{{height:10px;width:160px;border-radius:99px;background:linear-gradient(90deg,#0E7E7B,#54B62E);margin-bottom:22px}}
.tag{{font-family:'Space Mono',monospace;font-weight:700;font-size:22px;letter-spacing:.12em;text-transform:uppercase;color:{subcol}}}
.tag .h{{color:{taghl}}}
.url{{font-family:Sora;font-weight:700;font-size:30px;color:{eye};margin-top:6px}}
</style></head><body>
<div class="card">
  <div>{logo}</div>
  <div>
...
print("HTMLs gerados:", len(PIECES), "em", OUT)
print("\n".join(p["file"] for p in PIECES))
